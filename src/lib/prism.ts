 ...
 *
 *  In a PRISM project aiSLAP writes into `<entity>/Renders/AI`, and that media
 *  root — not the entity folder — is what the session carries as its shot path,
 *  so every gallery/version/tag path below it works unchanged. The cost is that
 *  `basename(shotPath)` reads "AI" instead of the shot name, which is what
