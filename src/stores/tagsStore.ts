...
type State = {
  ...
  /** Registry model ids narrowing the gallery, ANDed with the other two and
   *  read as OR within itself — so two models can be compared side by side in
   *  one gallery. Empty = no filtering.
   *
   *  No ANY/ALL mode, unlike tags: an image has exactly one model, so ALL of
   *  two ids would always be empty. */
...
type Actions = {
  ...
  /** Adds or removes one model id from the filter. Accumulates, like tags. */
...
/** Distinct model ids across both loaded views — same cache-on-array-identity
 *  trick as `discoveredUserNames`, since it walks the same images on the same
 *  rescans.
 *
 *  Ids, not labels: the registry resolves those (`lib/modelLabels.ts`), and
 *  keeping this a pure function of the two arrays is what makes the identity
 *  cache sound — folding the registry in would add a third key that is stable
 *  only by convention. A plain `Set` rather than the first-spelling-wins `Map`
 *  the other two use, because a registry slug has no spelling variance to
 *  preserve.
 *
 *  Sorted by *id*, for determinism only. The filter bar re-sorts by resolved
 *  label, which is the order the user actually reads. */
...
function discoveredModelIds(
  columns: { images: { modelId?: string }[] }[],
  taggedGroups: { shots: { images: { modelId?: string }[] }[] }[],
): string[] {
  if (
    discoveredModelsCache &&
    discoveredModelsCache.columns === columns &&
    discoveredModelsCache.taggedGroups === taggedGroups
  ) {
    return discoveredModelsCache.ids;
  }
  const seen = new Set<string>();
  for (const c of columns)
    for (const i of c.images) if (i.modelId) seen.add(i.modelId);
  for (const s of taggedGroups)
    for (const sh of s.shots)
      for (const i of sh.images) if (i.modelId) seen.add(i.modelId);

  const ids = [...seen].sort();
  discoveredModelsCache = { columns, taggedGroups, ids };
  return ids;
}

export function useDiscoveredModelIds(): string[] {
  const columns = useSessionStore((s) => s.columns);
  const taggedGroups = useSessionStore((s) => s.taggedGroups);
  return useMemo(
    () => discoveredModelIds(columns, taggedGroups),
    [columns, taggedGroups],
  );
}

/** The two non-tag narrowings both surfaces apply client-side: `generatedBy`
 *  (single-select) and model (OR over a set).
 *
 *  Split out of `matchesFilter` because the tag view gets its *tag* filtering
 *  from `project_tag_scan` server-side and must apply only these two on top —
 *  one definition of "this user AND one of these models", used by both, so the
 *  two surfaces cannot drift. */
export function matchesScope(
  image: Pick<GalleryImage, "generatedBy" | "modelId">,
  scope: { user: string | null; modelIds: string[] },
): boolean {
  if (scope.user && !(image.generatedBy && eq(image.generatedBy, scope.user)))
    return false;
  // Exact, unlike the user filter: a model id is a registry slug, not
  // user-typed or OS-supplied text, so there is no case to fold. Matches how
  // AUDIT's report filter treats `modelId` (see lib/costReport.ts).
  if (
    scope.modelIds.length > 0 &&
    !(image.modelId && scope.modelIds.includes(image.modelId))
  )
    return false;
  return true;
}

/** Does an image satisfy the whole active filter — `matchesScope` ANDed with
 *  its tags against `filter.tags` (ANY/ALL per `filter.mode`). */
export function matchesFilter(
  image: Pick<GalleryImage, "tags" | "generatedBy" | "modelId">,
  filter: {
    tags: string[];
    mode: TagFilterMode;
    user: string | null;
  ...
  if (!matchesScope(image, filter)) return false;
  if (filter.tags.length === 0) return true;
  const has = (w: string) => (image.tags ?? []).some((t) => eq(t, w));
  return filter.mode === "all" ? filter.tags.every(has) : filter.tags.some(has);
}
...
export function createTagsStore(tab: TabStores) {
  ...
  // No afterFilterChange() here either, for the same reason.
  toggleModelFilter(modelId) {
    set((s) => ({
      activeModelFilter: s.activeModelFilter.includes(modelId)
        ? s.activeModelFilter.filter((m) => m !== modelId)
        : [...s.activeModelFilter, modelId],
    }));
  },

  clearModelFilter() {
    set({ activeModelFilter: [] });
  },
